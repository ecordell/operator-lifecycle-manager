...
func (i *OperatorSourceInfo) String() string {
	return fmt.Sprintf("%s/%s in %s/%s", i.Package, i.Channel, i.CatalogSource, i.CatalogSourceNamespace)
}

var ExistingOperator = OperatorSourceInfo{"", "", "", ""}
...
func NewOperatorFromBundle(bundle *opregistry.Bundle, sourceKey CatalogKey) (*Operator, error) {
	...
	return &Operator{
		...
		sourceInfo:   &OperatorSourceInfo{
			...
			CatalogSource: sourceKey.Name,
			CatalogSourceNamespace: sourceKey.Namespace,
		},
	}, nil
}
