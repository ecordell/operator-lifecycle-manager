...
func (e *NamespaceGenerationEvolver) addNewOperators(add map[OperatorSourceInfo]struct{}) error {
	for s := range add {
		bundle, key, err := e.querier.FindPackage(s.Package, s.Channel, CatalogKey{Name: s.CatalogSource, Namespace: s.CatalogSourceNamespace})
...
func (e *NamespaceGenerationEvolver) queryForRequiredAPIs() error {
	...
	for api := range e.gen.UncheckedAPIs() {
