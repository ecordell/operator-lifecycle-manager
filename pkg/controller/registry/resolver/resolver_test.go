...
func existingOperator(namespace, operatorName, pkg, channel, replaces string, provided, required APISet) *v1alpha1.ClusterServiceVersion {
	bundleForOperator := bundle(operatorName, pkg, channel, replaces, provided, required)
...
func bundleSteps(bundle *opregistry.Bundle, ns string) []*v1alpha1.Step {
	stepresources, err := NewStepResourceFromBundle(bundle, ns)
	if err != nil {
		panic(err)
	}
...
func subSteps(namespace, operatorName, pkgName, channelName string, catalog CatalogKey) []*v1alpha1.Step {
	stepName := fmt.Sprintf("%s/%s in %s/%s", pkgName, channelName, catalog.Name, catalog.Namespace)
	sub := &v1alpha1.Subscription{
		ObjectMeta: metav1.ObjectMeta{
			GenerateName: pkgName,
			Namespace:    namespace,
		},
		Spec: &v1alpha1.SubscriptionSpec{
			Package:                pkgName,
	...
	stepresource, err := NewStepResourceFromObject(sub, stepName)
	if err != nil {
		panic(err)
	}
...
func TestNamespaceResolver_ResolveSteps(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			bundlesInCatalog: []*opregistry.Bundle{
				bundle("a.v1", "a", "alpha", "", nil, nil),
			},
			out: out{
				steps: [][]*v1alpha1.Step{
					bundleSteps(bundle("a.v1", "a", "alpha", "", nil, nil), namespace),
				},
				subs: []*v1alpha1.Subscription{
		...
		},
		{
			...
			bundlesInCatalog: []*opregistry.Bundle{
				bundle("b.v1", "b", "beta", "", Provides1, nil),
				bundle("a.v1", "a", "alpha", "", nil, Requires1),
			},
			out: out{
				steps: [][]*v1alpha1.Step{
					bundleSteps(bundle("a.v1", "a", "alpha", "", nil, Requires1), namespace),
					bundleSteps(bundle("b.v1", "b", "beta", "", Provides1, nil), namespace),
		...
		},
		{
			...
			clusterState: []runtime.Object{
				...
				existingOperator(namespace, "a.v1", "a", "alpha", "", Provides1, nil),
			},
			bundlesInCatalog: []*opregistry.Bundle{
				bundle("a.v1", "a", "alpha", "", Provides1, nil),
			},
			out: nothing,
		},
		{
			...
			bundlesInCatalog: []*opregistry.Bundle{
				bundle("a.v2", "a", "alpha", "a.v1", Provides1, nil),
				bundle("a.v1", "a", "alpha", "", Provides1, nil),
			},
			out: out{
				steps: [][]*v1alpha1.Step{
					bundleSteps(bundle("a.v2", "a", "alpha", "a.v1", Provides1, nil), namespace),
				},
				subs: []*v1alpha1.Subscription{
		...
		},
		{
			...
			out: out{
				steps: [][]*v1alpha1.Step{
					bundleSteps(bundle("a.v1", "a", "alpha", "", Provides1, nil), namespace),
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resolver := NewNamespaceResolver(namespace, informerFactory)
			querier := NewFakeSourceQuerier(map[CatalogKey][]*opregistry.Bundle{catalog: tt.bundlesInCatalog})
			steps, subs, err := resolver.ResolveSteps(querier)
			require.Equal(t, tt.out.err, err)
			require.EqualValues(t, expectedSteps, steps)
			require.Equal(t, tt.out.subs, subs)
		})
	}
}
